...
int main(int argc, char const *argv[]) {
  ...
  ProtectedBlockQueue* queue = writer.getQueue();
  FileCompressor fileCompressor(&file, queue, *argv[1] - '0', 1, 0); //AL PRIMER HILO MANDAR 0 DE ULTIMO ARG
  fileCompressor.start();
  writer.write();
  fileCompressor.join();
  /*std::vector<ProtectedBlockQueue*> queueVector;
  queueVector.push_back(new ProtectedBlockQueue(*argv[3] - '0'));
